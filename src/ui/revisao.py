...
class Revisao(ctk.CTkFrame):
    ...
    def _build(self):
        ...
        # Seção de feedback (aparece após revelar)
        ...
        self._criar_btn_feedback(btns, "✓", "Conexões Claras", "Lembrei bem", "#14532d", "#16a34a", "claro", 0)
        self._criar_btn_feedback(btns, "−", "Clareando", "Com esforço", "#451a03", "#d97706", "clareando", 1)
        self._criar_btn_feedback(btns, "✕", "Ineficaz", "Não lembrei", "#4c0519", "#db2777", "ineficaz", 2)

    def _criar_btn_feedback(self, parent, icone, titulo, subtitulo, bg, cor, feedback, col):
        btn = ctk.CTkFrame(parent, fg_color=bg, corner_radius=14, width=180, height=90, cursor="hand2")
        btn.grid(row=0, column=col, padx=8)
        btn.pack_propagate(False)

        ctk.CTkLabel(btn, text=icone, font=ctk.CTkFont(size=18), text_color=cor).pack(pady=(16, 2))
        ctk.CTkLabel(btn, text=titulo, font=ctk.CTkFont(size=13, weight="bold"), text_color=cor).pack()
        ctk.CTkLabel(btn, text=subtitulo, font=ctk.CTkFont(size=11), text_color=COR_MUTED).pack()

        btn.bind("<Button-1>", lambda e: self.responder(feedback))
        for w in btn.winfo_children():
            w.bind("<Button-1>", lambda e: self.responder(feedback))
    ...
    def mostrar_atual(self):
        ...
        self.tag_categoria.configure(text=f"{sem}º semestre" if cat == "semestre" else "auto-estudo")
