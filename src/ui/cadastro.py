...
import uuid # Para criar um ID único para cada assunto
...
class Cadastro(ctk.CTkFrame):
    ...
    def _build_ui(self):
        ...
        # Títulos superiores
        ctk.CTkLabel(self.main_container, text="Novo Assunto", 
                     font=ctk.CTkFont(size=28, weight="bold"), text_color="white").pack(pady=(0, 5))
        ctk.CTkLabel(self.main_container, text="Adicione um tópico ao seu ciclo de revisão.", 
                     font=ctk.CTkFont(size=14), text_color=COR_MUTED).pack(pady=(0, 30))

        # Card Principal (Fundo escuro arredondado)
        form = ctk.CTkFrame(self.main_container, fg_color=BG_CARD, corner_radius=20, width=420)
        ...
        self.seg_button = ctk.CTkSegmentedButton(inner, values=["Semestre", "Auto-Estudo"],
                                                variable=self.categoria_var,
                                                fg_color=BG_INPUT, selected_color=COR_ROXA,
                                                unselected_color=BG_INPUT, text_color="white",
                                                height=35)
        self.seg_button.pack(fill="x", pady=(0, 20))

        # 2. Matéria (Placeholder limpo)
        self._label(inner, "Matéria")
        self.entry_materia = self._input(inner, "Digite o nome da disciplina...")

        # 3. Assunto (Placeholder limpo)
        self._label(inner, "Assunto")
        self.entry_assunto = self._input(inner, "O que vai estudar?")

        # 4. Anotação
        self._label(inner, "Anotação (opcional)")
        self.txt_anotacao = ctk.CTkTextbox(inner, fg_color=BG_INPUT, border_color="#2d1b4e", 
                                          height=80, width=340, corner_radius=10, text_color=COR_TEXTO)
        self.txt_anotacao.pack(pady=(0, 20))

        # 5. Botões de Ação
        self.btn_salvar = ctk.CTkButton(inner, text="Salvar", font=ctk.CTkFont(weight="bold"),
        ...
        self.btn_voltar = ctk.CTkButton(inner, text="Voltar", fg_color="transparent", 
        ...
        # 6. Semestre (Lista expandida até o 8º como solicitado)
        self._label(inner, "Semestre")
        valores_semestre = [f"{i}º semestre" for i in range(1, 9)]
        self.combo_semestre = ctk.CTkComboBox(inner, values=valores_semestre,
                                             fg_color=BG_INPUT, border_color="#2d1b4e",
                                             button_color=COR_ROXA, dropdown_fg_color=BG_INPUT, width=340)
        self.combo_semestre.pack(fill="x")
        self.combo_semestre.set("1º semestre") # Valor padrão
    ...
    def _label(self, parent, txt):
        ...
        ctk.CTkLabel(parent, text=txt, font=ctk.CTkFont(size=12, weight="bold"), 
                     text_color=COR_MUTED).pack(anchor="w", pady=(0, 5))

    def _input(self, parent, placeholder):
        """Auxiliar para criar campos de entrada padronizados."""
        entry = ctk.CTkEntry(parent, placeholder_text=placeholder, fg_color=BG_INPUT, 
                            border_color="#2d1b4e", height=40, corner_radius=10, text_color=COR_TEXTO)
    ...
    def salvar_dados(self):
        ...
        # Montagem do objeto conforme esperado pelo repositório
        novo_item = {
            "id": str(uuid.uuid4()),
            "materia": materia,
            "assunto": assunto,
        ...
        # Gravação no dados.json
        adicionar_item(novo_item)
        
        # Navega de volta para a tela anterior (Dashboard ou Meus Assuntos)
        self.on_voltar()