...
APIUso na aplicaçãoWikipedia REST APIResumo automático ao cadastrar assuntoOpen-MeteoClima atual na dashboard (temperatura, condição)UnsplashImagem de capa por assunto cadastradoGitHub APIPerfil e repos do estudante na dashboardNager.DatePróximos feriados brasileirosREST CountriesDados de países (bandeira, capital, população)
...
Lint
bashflake8 src/ tests/ web_app.py

Entrega Intermediária

 Integração com 6 APIs públicas
 39 testes automatizados (unitários + integração)
 Deploy no Render com link público
 GitHub Issue → Branch entrega-intermediaria → PR → Merge
 CI atualizado (GitHub Actions)
